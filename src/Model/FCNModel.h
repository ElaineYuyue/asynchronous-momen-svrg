...
class FCNModel : public Model {
 ...
 public:
  ...
  double ComputeLoss(Datapoint *datapoints, double& accuracy) override {
    ...
    mat w_1 = vec_2_mat(momentum, begin, dims[0], dims[1]);

	begin += dims[0] * dims[1];
	//mat w_2 = vec_2_mat(model, begin, dims[1], dims[2]);

    mat w_2 = vec_2_mat(momentum, begin, dims[1], dims[2]);
  ...
  void PrecomputeCoefficients(Datapoint *datapoints, Gradient *g, std::vector<double> &local_momentum, const std::vector<int> &left_right ) override {
	...
	int begin=0;
	mat w_1 = vec_2_mat(local_momentum, begin, dims[0], dims[1]);
	begin += dims[0] * dims[1];
	mat w_2 = vec_2_mat(local_momentum, begin, dims[1], dims[2]);
