...
class LSL2L1Model : public Model {
 ...
 public:
  ...
  double ComputeLoss(Datapoint *datapoints, double& auc) override {
    ...
    mat w = vec_2_mat(momentum, 0, dims[0], dims[1]);
  ...
  virtual double ComputeRegularization() override {
    ...
    for(int i = 0; i < momentum.size(); i++) {
      ...
      regloss += std::abs(momentum[i]) * FLAGS_l1_lambda + 0.5 * pow(momentum[i], 2) * FLAGS_l2_lambda;
	}
	return regloss;
  }
...
// }
   ...
   virtual void ProximalOperator(std::vector<double> &local_momentum, double gamma) override {
       for (int i=0; i<local_momentum.size(); i++) {
           double val = local_momentum[i];
           double sign = val > 0 ? 1 : -1;
           local_momentum[i] = sign * fmax(std::abs(val) - gamma, 0);
       }
   }
 ...
 std::vector<double> & MomentumData() override {
    return momentum;
  }
  ...
  void PrecomputeCoefficients(Datapoint *datapoints, Gradient *g, std::vector<double> &local_momentum, const std::vector<int>& left_right) override {
      int size = left_right[1] - left_right[0];
      if (g->coeffs.size() != n_coords) g->coeffs.resize(n_coords);
      mat w = vec_2_mat(local_momentum, 0, dims[0], dims[1]);
      sp_mat grad(dims[0], dims[1]);
      mat o(size, dims[1]);
      mat dldo(size, dims[1]);
      mat dx(size, dims[0]);
  ...
  //}
  //
  virtual void ComputeL2Gradient(Gradient *g, std::vector<double> &local_momentum) override{
      for (int i=0; i<NumParameters(); i++) {
          g->coeffs[i] += FLAGS_l2_lambda * local_momentum[i];
      }
  }
