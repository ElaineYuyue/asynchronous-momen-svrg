...
class MULTICLASSTRACEModel : public Model {
 ...
 public:
  ...
  double ComputeLoss(Datapoint *datapoints, double& accuracy) override {
    ...
    mat w_1 = vec_2_mat(momentum, 0, dims[0], dims[1]);
  ...
  virtual double ComputeRegularization() override {
	...
	for(int i=0; i<momentum.size(); i++){
	  regloss += 0.5 * pow(momentum[i], 2) * FLAGS_l2_lambda;
	}
	// nuclear norm
	mat w_1 = vec_2_mat(momentum, 0, dims[0], dims[1]);
	mat U, V;
  ...
  virtual void ProximalOperator(std::vector<double> &local_momentum, double gamma) override{
	// trace norm operator.
	mat w_1 = vec_2_mat(local_momentum, 0, dims[0], dims[1]);
	mat U, V;
	vec s;
	svd(U, s, V, w_1);
	mat tmp_s(w_1);
